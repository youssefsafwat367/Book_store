...
class OrderController extends Controller
{
    ...
    public function index()
    {
        ...
        if (auth()->user() != NULL) {
            ...
            return view('front.orders', ['branches' => $branches, 'carts' => $carts, 'orders' => $orders]);
        } else {
            return view('front.orders', ['branches' => $branches]);
        }
    }
    ...
    public function create(Request $request , $total)
    {
            ...
            foreach ($products as $product) {
                ...
                if ($cart->cart_products[0]->product_id == $product->id) {
                    if ($cart->cart_products[0]->quantity > $product->stock) {
                        return redirect()->route('homePage')->with('danger', "The available quantity for  product : $product->name is  $product->stock now");
                    }
                }
            }
        }
        ...
        return redirect()->route('homePage')->with('success', 'The order is Confirmed');
    }
    /**
    ...
    public function checkout($total)
    {
        ...
        if (auth()->user() != NULL) {
            $carts = Cart::where('user_id', '=', auth()->user()->id)->where('status', '=', 'قيد التنفيذ')->get();
            $orders = Order::where('user_id', '=', auth()->user()->id)->get();
            return view('front.checkout', ['branches' => $branches, 'carts' => $carts, 'orders' => $orders, 'total'=>$total]);
        } else {
            return view('front.checkout', ['branches' => $branches, ['total'=>$total]]);
        }
    }
    ...
    public function order_details(string $id)
    {
        ...
        if (auth()->user() != NULL) {
            $carts = Cart::where('user_id', '=', auth()->user()->id)->where('status', '=', 'قيد التنفيذ')->get();
            return view('front.order-details', ['order' => $order, 'branches' => $branches, 'carts' => $carts , 'order_products' => $order_products]);
        } else {
            return view('front.orders', ['branches' => $branches]);
        }
    }
    public function confirm($id){
        ...
        return back()->with('success' , "Order Is Confirmed successfully") ;
    }
    public function received($id)
    {
        $order = Order::find($id)->update(['status' => 'تم الاستلام']);
        return back()->with('success', "Order Is Received successfully");
    }
}
